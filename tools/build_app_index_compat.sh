#!/bin/sh
# 编译 AppIndexCompat.java 并同步生成 smali/com/tsf/shell/compat/。
set -eu

script_dir=$(CDPATH= cd -- "$(dirname "$0")" && pwd)
repo_root=$(CDPATH= cd -- "$script_dir/.." && pwd)
src_root="$repo_root/src-helpers"
build_dir="$repo_root/build/app-index-compat"
...
javac -source 1.8 -target 1.8 -encoding UTF-8 \
  ...
  -d "$build_dir/classes" \
  "$src_root"/com/tsf/shell/compat/AppIndexCompat.java
...
cp "$build_dir"/smali/com/tsf/shell/compat/AppIndexCompat.smali "$out_smali_dir/"

echo
echo "smali written to: $out_smali_dir/AppIndexCompat.smali"
