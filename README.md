...
TSF Launcher 是一代经典且极具创新的 3D 交互桌面应用，但由于长期停更（最后版本停留于 targetSdkVersion 26 / Android 8.0），在现代 Android 系统（特别是 Android 12-16）上面临着严重的兼容性问题。
...
- [ ] 修复现代 Android 系统下的启动崩溃问题
- [ ] 适配最新的 Android 权限模型（存储、通知、前台服务等）
- [ ] 适配 Android 12+ 的 `android:exported` 等组件声明约束
...
由于本项目引入了成熟的工具链，你可以使用以下脚本快速进行开发验证：
...
为了在新版 Android 上顺利运行，后续的逆向修改将主要集中在：
- `AndroidManifest.xml` (补充 `POST_NOTIFICATIONS` 权限，补齐所有四大组件的 `android:exported` 属性规范)。
- `smali/com/tsf/shell/Home.smali` 以及相关的初始化入口。
- `smali/com/tsf/shell/services/ForegroundService.smali` 等服务组件 (适配 Android 8.0+ 及更高版本对前台服务启动的严格限制)。
